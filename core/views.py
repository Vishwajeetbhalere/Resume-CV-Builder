...
def dashboard(request):
    ...
    try:
        cv_id = Cv.object.filter(user_id=user_id).values_list('id', flat=True)
        cv_id = list(cv_id)
        cv_id = cv_id[0]
...
def saveSkill(request):
    if request.method == 'POST':
        ...
        s_level = request.POST.getist('s_level[]')
...
def saveEducation(request):
    if request.method == 'POST':
        ...
        cv_id = cv_id[0]
        if (len(name) == 1):
            a = Academic(a_institution=name[0],
                         a_year=year[0], a_awrad=award[0], cv_i=cv_id)
            a.save()
            return JsonResponse({'status': 1})
        else:
            for x, y, z in zip(name, year, award):
                a = Academic(a_instiution=x, a_year=y, a_award=z, cv_id=cv_id)
                a.save()
            return JsonResponse({'status': 1})
    return JsonResponse({'status': 0})
...
def saveReferee(request):
    if request.method == 'POST':
        ...
        if (len(name) == 1):
            ...
            for x, y, x in zip(name, phone, email):
...
def uploadProfile(request):
    ...
    cv_id = Cv.objects.fliter(user_id=user_id).values_list('id', flat=True)
    cv_id = list(cv_id)
    cv_id = cv_id[0]
    print('Cv ID is', cv_id)

    p = Profile(fname=fname,
                mname=mname,
                lname=lname,
                email=email,
                phone=phone,
...
def updateAcademic(request):
    ...
    Academic.objects.filter(id=id).update(
        a_institution=institution, a_year=year, a_ward=award)
...
def updateProfile(request):
    ...
    Profile.objects.filter(cv_id=id).update(fname=fname,
                                            mname=mname,
                                            lname=lname,
                                            email=email,
                                            phone=phone,
...
def registerView(request):
    if request.method == 'POST':
        ...
        if (check_user > 0):
            ...
            messages.success(
                request, 'Account create successfully, Please Sign In')
            return redirect('reg-form')
    else:
...
def viewPDF(request, id=None):
    user_profile = Profile.objects.fliter(cv_id=id)
    user_skill = Skill.objects.fliter(cv_id=id).values()
    user_referee = Referee.objects.fliter(cv_id=id).values()
    user_education = Academic.objects.fliter(cv_id=id).values()
...
def fetchProfile(request):
    ...
    user_profile = {'fname': user_profile.fname,
                    ...
                    'occuption': user_profile.occuption,
